...
(function (document, window, $) {
  ...
  var Show = {
    ...
    _checkValue: function(currentValue, requiredValue) {
      if(requiredValue.indexOf(Show.controlSeperator) > -1) {
        requiredValue = requiredValue.split(Show.controlSeperator);
    ...
    showHideCheckSelect: function($input, $element, instant) {
      instant = instant || false;
      var showField = Show._checkValue($input.val(), $element.attr("data-show-option"));
    ...
    showHideCheckInput: function($input, $element, instant) {
      ...
      if(showType === "*") {
        ...
        // Exact matching
        showField = Show._checkValue(valueLower, valueToMatch);
      }
