...
(function (document, window, $) {
  ...
  $(document).on("ornament:refresh", function () {
      ...
      scaffoldMobileMenu: function(){
        ...
        mobileNav.tray.find("li").not(mobileNav.menuDescriptionClass).each(function(){
          ...
          if($parentNode.children("div").length > 0) {
            $parentNode.addClass(mobileNav.hasChildren);
          }
        });
    ...
    mobileNav.tray.find("." + mobileNav.hasChildren).children("a").on("click", function(e) {
      e.preventDefault();
      ...
      mobileNav.currentLevel = mobileNav.currentLevel + 1;
      mobileNav.tray.attr("data-level", mobileNav.currentLevel);
      mobileNav.updateMobileMenuHeightWithDelay();
    });

    // Making back buttons work
    mobileNav.tray.find("[data-mobilenav-back]").on("click", function(e){
      e.preventDefault();
      mobileNav.goBack();
    });
