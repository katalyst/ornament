...
(function (document, window, Ornament, Utils) {
  ...
  var Drilldown = {
    ...
    // Internal data storage to replace $.data()
    // dataStore: [{
    //   element: <node>,
    //   data: {},
    // }]
    ...
    // Drilldown.getData(element, "firstChild") => node
    getData: function(element, key) {
      var data = false;
      Drilldown.$dataStore.find(function(datum){
    ...
    // Drilldown.setData(element, "firstChild", element.childNodes[0]);
    setData: function(element, key, value) {
      var existingData = Drilldown.getData(element);
    ...
    _getParentOfList: function($list) {
      return Drilldown.getData($list, "parentList");
    },
    ...
    _isSplitDrilldown: function($container) {
      return Drilldown.getData($container, "drilldownSplitLinks")
    },
    ...
    _setParentListPast: function($list) {
      if(Drilldown.getData($list, "parentList")) {
        var $parent = Drilldown._getParentOfList($list);
    ...
    _clearClasses: function($list) {
      $list.classList.remove(Drilldown.classes.pastList, Drilldown.classes.futureList, Drilldown.classes.currentList);
    },
    ...
    _setDataOnContainer: function($container, $drilldown) {
      ...
      if($drilldown.hasAttribute(Drilldown.selectors.overviews)) {
        Drilldown.setData($container, "drilldownOverviews", true);
      }
      if($drilldown.hasAttribute(Drilldown.selectors.titles)) {
        Drilldown.setData($container, "drilldownTitles", true);
      }
      if($drilldown.hasAttribute(Drilldown.selectors.splitLinks)) {
        Drilldown.setData($container, "drilldownSplitLinks", true);
      }
    },
    ...
    _scaffold: function($drilldown){
      ...
      Drilldown.setData($container, "firstList", $firstList);
        ...
        $uls.map(function($nestedUl){
          ...
          if(Drilldown._isSplitDrilldown($container)) {
            ...
            if(Drilldown.useOrnamentSvg) {
              $forwardButton.innerHTML = Ornament.icons[Drilldown.icons.forward];
            }
            ...
            // Add parent svg icon 
            if(Drilldown.useOrnamentSvg) {
              $newParentLink.appendChild(Ornament.icons[Drilldown.icons.forward]);
            }
          ...
          Drilldown.setData($nestedList, "parentList", $list);
        });
      }
    ...
    _prependBackButton: function($ul) {
      ...
      if(Drilldown.useOrnamentSvg) {
        $buttonInner.insertBefore(Ornament.icons[Drilldown.icons.back], $buttonInner.firstChild);
      }
    ...
    _prependDescriptionBlock: function($container, $parentLink, $ul) {
      ...
      var hasTitle = Drilldown.getData($container, "drilldownTitles");
      var hasOverview = Drilldown.getData($container, "drilldownOverviews");
