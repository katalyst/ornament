    ...
    create: function(checkboxSelector, fields) {
      ...
      var toggleFields = function() {
        for (var i = fieldKeys.length - 1; i >= 0; i--) {
          var $shippingField = document.getElementById(fields[fieldKeys[i]]);

          // if checked, get keys and copy values in to fields
          if($checkbox.checked) {
            var $billingField = document.getElementById(fieldKeys[i]);
      ...
      // bind change event
      Ornament.U.bindOnce($checkbox, "change", toggleField);
    },
