/* ==================================================
  ...
  [x] jQuery plugin with options for callbacks

  Week 2:
  [x] Pass data attributes through as partial settings
  [x] Create proof of concept using callbacks to generate form fields
  [x] File Type Validations
  [x] Update workflow to allow for adding and removing of images rather than over-write
  [x] Undo delete image
  [x] Basic browse functionality for local file over DND method

  Week 3:
  [x] Re-order gallery images
  [x] Save file to web storage and return an id, assign image to field
  [x] Progress Bar when uploading

  Week 4:
  [x] Fixed browse button
  [x] Move upload process in to core rather than callback
  [x] UI/UX 
  [x] Thumbnails for popular non-image types
  [x] Re-ordering updates hidden form field order of ids
  [x] Removing image needs to remove from hdiden field
  [x] Undo needs to re-add image-id to hidden field

  TO DO:
  [ ] Existing images need to inject image id in to hidden field
  [ ] Multiple image uploads for nested models
  [ ] Convert mime type matches to a key pair array with a callback for custom file type matches

  ENHANCEMENTS:
  [ ] Upload from other sources (camera, facebook, dropbox, google drive)
  [ ] S3 / CORS
  [ ] Being able to keep the image in memory for error pages
  ...
  FILE REQUIREMENTS
  * /assets/javascripts/components/image-upload.js (add to application.js)
  ...
  * /views/koi/crud/_form_field_image.html.erb
  ...
  MODEL MODIFICATIONS REQUIRED (see user.rb)
  assign_images function
  assign_images attr_accessor
  ...
  $.fn.extend({
    katFileUpload: function(options) {
      ...
      return this.each(function() {
        ...
        $dropZoneText.append("Drop files here or ").append($dropZoneBrowse).append(" for a file instead.");
        ...
        // Add existing images
        if(existingImage) {
          createThumbnail(existingImage);
        }

        // creating a rubbish bin and undo button for undo deletion
        var $undoButton = $("<div class='file-upload--undo'><span>Undo Remove</span></div>");
        ...
        var updateDropZoneIfMaxed = function(){
          ...
          if ( getGalleryCount() >= maxCount && maxCount != 0 ) {
            debug("Gallery is maxed, no more uploads are allowed.");
            $dropZone.find(".file-upload--message").remove();
            $dropZone.append("<div class='file-upload--message'>Maximum images reached.</div>");              
          } else {
        ...
        var getInfoAboutFile = function(files){
          ...
          for (var i = 0, f; f = files[i]; i++) {
            ...
            // Clearing previously uploaded image(s)
            // no longer needed, using add/remove behaviour now instead
            // $dropZoneContainer.find(".thumbnail-container").remove();
...
$(function(){
  $(".file-upload").katFileUpload({
    afterImageUpload: function(f,thumbnail,gallery,thumbnailTarget,$fileElement){



    }
  });
  $(".file-upload-with-callback").katFileUpload({
    afterImageUpload: function(f,thumbnail,gallery,thumbnailTarget){
      var $extraFields = $("<div>").addClass("file-upload-with-callback--fields");
      // TODO: create unique names and ids
      $extraFields.append( $("<label for=''>Caption</label>") );
      $extraFields.append( $("<input type='text' name='' id='' />") );
      $extraFields.append( $("<label for=''>Alt Text</label>") );
      $extraFields.append( $("<input type='text' name='' id='' />") );
      $(thumbnail).append($extraFields);
    },
    afterFileUpload: function(f,thumbnail,gallery,thumbnailTarget){
      return this.afterImageUpload(f,thumbnail,gallery,thumbnailTarget);
    }
  });
});
