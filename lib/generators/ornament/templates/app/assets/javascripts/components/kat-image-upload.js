  ...
  $.fn.extend({
    katFileUpload: function(options) {
      ...
      return this.each(function() {
        ...
        var croppable = $thisField.is("[data-file-crop-ratio]");
        var $cropModal = false;
        ...
        if(croppable) {
          ...
          // Update crop ratio
          cropRatio = $thisField.attr("data-file-crop-ratio");
        }
        ...
        var createThumbnail = function(image){
          ...
          //TODO: What is this?
          // Update thumbnail sizing based on crop ratio
          var cropRatio = $thisField.attr("data-crop-ratio") || "1/1";
          if(cropRatio !== "1/1") {
            var cropRatio = cropRatio.split("/");
            var cropRatio = cropRatio[0] / cropRatio[1];
            $thumbnailContainer.parent().width(80 * cropRatio);
          }
        ...
        var manageCoords = function(c){
          tempCropString = Ornament.uploader.manageCoords(c);
        }
        ...
        var centreCoords = function() {
          var x = 0;
          var y = 0;
          var ratio = previewWidth / previewHeight;
          var offset = 0;
          if (previewHeight > previewWidth) {
            offset = (previewHeight / 2) - ((previewHeight * ratio) * 0.5);
            y = offset;
          } else {
            offset = (previewWidth / 2) - ((previewWidth / ratio) * 0.5);
            x = offset;
          }
          return { x: x, y: y };
        }
        ...
        var initialiseCrop = function($image, currentCrop){
          $image.Jcrop({
            ...
            aspectRatio: parseInt(cropRatio),
        ...
        // TODO: Switch this to state-based display
        var showRemoveButton = function() {

          // Hide basic message
          $uploader.find("[data-file-message]").hide();
          ...
          $thumbnailRemove.off("click").on("click", function(e){
            ...
            $uploader.find("[data-file-message]").show();
            // remove id from field
            $hiddenField.val("");
            // remove crop string field
            $cropStringField.val("");
            // callback for crop etc.
            $thisField.trigger("imageRemoved");
          });

          // Show crop option if croppable
          if(croppable) {
        ...
        if(croppable) {
          ...
          $thisField.trigger("uploader:close-crop-modal", function(){
            closeCropModal();
          });
        }
        ...
        $dropZoneBrowse = $("<a href='#' class='file-upload--browse-button'>browse</a>");
        $dropZoneText.append($("<span data-file-message>"+options.browseMessage+"</span>").append($dropZoneBrowse));
            ...
            return function(e) {
              ...
              if(options.demo) {
                ...
                var progressInterval = setInterval(function(){
                  progressCounter += 20;
                  $progressBar.val(progressCounter);
                }, 1000);
        ...
        var uploadFinished = function(){
          $dropZone.removeClass("saving uploading").addClass("uploaded");
          $progressBar.hide();
          ...
          // if (imageFile) {
          //   options.afterImageUpload(f,thumbnail,gallery,thumbnailTarget,$fileElement);
          // } else {
          //   options.afterFileUpload(f,thumbnail,gallery,thumbnailTarget,$fileElement); 
          // }
          ...
          // Callbacks
          $thisField.trigger("afterUpload", [ $thumbnailContainer ]);

          // Dragonfly remove support
          // $thisField.siblings(".form--file--remove").prop("checked", false);
        }
