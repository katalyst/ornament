...
(function (document, window, $) {
  ...
  $(document).on("ornament:refresh", function () {
    ...
    var onlyOneTooltipAtATime = false;
    ...
    var hideAllTooltips = function(){
      $("[data-tooltip-from]").remove();
    }

    // Event to hide all tooltips
    $(document).on("ornament:toolips:hide", function(){
      hideAllTooltips();
    });

    // General "show" function
    var showTooltip = function($tooltip, $anchor) {
      if(onlyOneTooltipAtATime) {
        hideAllTooltips();
      }
    ...
    $tooltipAnchors.each(function (i) {
      ...
      if( toggleTooltips || $anchor.is("[data-tooltip-toggle]") ) {
        ...
        // Toggle tooltips when tabs are changed
        $(document).on("ornament:tab-change", function(){
          if($anchor.parents(".tabs") && !$anchor.is(":visible")) {
            hideTooltip($outer);
          }
        });
