    ...
    katSelector: function(options) {
      ...
      var updateSelector = function($selector, list) {
        ...
        $.each(list, function(i){
          ...
          label.append("<div class='table-cell'>" + template + "</div>");
          label.wrapInner("<div class='table' />");
          item.append(label);
          item.find("input").wrap("<div class='table-cell' />");
      ...
      var updateSelectorValue = function($selector) {
        ...
        // Callback
        if(options.onChange) {
          options.onChange($selectedLabel, value);
        }
      }
