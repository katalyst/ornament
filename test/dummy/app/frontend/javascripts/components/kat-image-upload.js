...
import "../ornament/utilities/uploader-helpers";
...
(function($){
  ...
  $.fn.extend({
    katFileUpload: function(options) {
      ...
      // Feature Detection
      // You can use modernizr to test for file API support if you require legacy support
      ...
      return this.each(function() {
        ...
        var setState = function(stateString) {
          ...
          if(cropButtonVisible) {
            $cropButton.off("click").on("click", function(e){
              ...
              // Open modal
              var popupOptions = $.extend(true, {}, Ornament.C.Lightbox.defaults);
              popupOptions.mainClass = "lightbox--main lightbox__cropper";
              popupOptions.closeOnBgClick = false;
              popupOptions.items = {
                type: "inline",
                src: $cropModal
              };
              popupOptions.callbacks = popupOptions.callbacks || {};
              // Resize and add crop widget when opening modal
              var defaultCallbacks = $.extend({}, popupOptions.callbacks);
              popupOptions.callbacks.open = function(){
                defaultCallbacks.open(arguments);
        ...
        var getInfoAboutFile = function(files){
          ...
          for (var i = 0, f; f = files[i]; i++) {
            ...
            // Last modified date isn't enabled in all browsers (eg. Safari)
            // Try some attempts at getting a reasonable modified date
            try {
              var fileDateModified = f.lastModifiedDate.toLocaleDateString();
            } catch(e) {
              var fileDateModified = f.lastModifiedDate;
            } finally {
              var fileDateModified = "Unknown last modified date";
            }
